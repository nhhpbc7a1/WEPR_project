...
export default {
    ...
    async getFeaturedArticlesThisWeek() {
        return await db('Articles as A')
            .select('A.title', 'C.category_name', 'A.published_date', 'A.image_url', 'A.article_id', 'A.category_id', 'A.is_premium', db.raw('COUNT(V.view_id) as view_count'))
    ...
    async getMostViewedArticles() {
        return await db('Articles as A')
            ...
            .select('A.title', 'C.category_name', 'A.published_date', 'A.image_url', 'A.article_id', 'A.abstract', 'A.category_id', 'A.is_premium', db.raw('COUNT(V.view_id) as view_count'))
            .join('Categories as C', 'A.category_id', 'C.category_id')
            .leftJoin('Article_Views as V', 'A.article_id', 'V.article_id')
            .groupBy('A.article_id')
